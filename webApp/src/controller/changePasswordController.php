	...
	if(isset($_POST['submit'])){
		if(!empty($_POST['pass']) && !empty($_POST['nPass']) && !empty($_POST['cnPass'])){
			...
			if($newPassword == $confirmPassword){
				$getPassword = "SELECT password, previous_passowrd FROM employee WHERE nic='".$nic."'";
				...
				if(mysqli_num_rows($resultGetPassword)!= 0){
					while($rowPassword = mysqli_fetch_array($resultGetPassword)){
						...
						$dbPreviousPassword = $rowPassword['employee_previous_password'];
						if($dbPassword == $oldPassword){
							$updatePasswordQuery = "UPDATE employee SET password='".$newPassword."', previous_passowrd='".$dbPassword."', login_attempt='0', STATUS='1' WHERE nic='".$nic."'";
