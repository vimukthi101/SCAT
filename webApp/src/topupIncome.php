        ...
        <div style="padding:10px;"> 
			...
			<div class="form-horizontal">
            	<div class="form-group">
                    <label for="pass" class="control-label col-md-3">Top-Up Agent : </label>
                    <div class="col-md-8">
                    	<select onchange="showHint(this.value);" class="form-control" name="topup" id="topup">
                        	<?php
								$get = "SELECT employee_nic, topup_agent_id FROM topup_agent";
								$result = mysqli_query($con, $get);
								if(mysqli_num_rows($result)!=0){
									echo '<option disabled="disabled" selected="selected">Select A top-Up Agent</option>';
									while($row = mysqli_fetch_array($result)){
										$nic = $row['employee_nic'];
										$id = $row['topup_agent_id'];
										echo '<option value="'.$nic.'">'.$nic.' - '.$id.'</option>';
									}
								} else {
									echo '<option>Top-up Agents Not Added Yet.</option>';
								}
							?>
                        </select>
                	</div>
                ...
                <script>
					function showHint(str) {
						if (str.length == 0) { 
							...
							xmlhttp.open("GET", "getIncome.php?p=topup&q=" + str , true);
							xmlhttp.send();
						}
					}
				</script>
